...
public class EnderManuscript extends Item {
    private static final Component description = Component.translatable("item.irons_restrictions.ender_manuscript_desc").withStyle(ChatFormatting.GRAY);

    public EnderManuscript(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand pUsedHand) {
        if (player instanceof ServerPlayer serverPlayer) {
            PacketDistributor.sendToPlayer(serverPlayer, new OpenEnderScreenPacket(pUsedHand));
        }
